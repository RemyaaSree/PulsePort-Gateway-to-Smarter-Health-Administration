...
# function to perform various operations of the appointment module (according to user's selection)
def appointments():
    st.header('APPOINTMENTS')
    option_list = ['', 'Add appointment', 'Update appointment', 'Delete appointment', 'Show all appointments', 'Show appointments by patient']
    option = st.sidebar.selectbox('Select function', option_list)
    a = Appointment()
    if (option == option_list[1] or option == option_list[2] or option == option_list[3]) and verify_edit_mode_password():
        if option == option_list[1]:
...
def home():
    ...
    option = st.sidebar.selectbox('Select module', ['', 'Patients', 'Doctors', 'Prescriptions', 'Medical Tests', 'Departments', 'Appointments'])
    if option == 'Patients':
        patients()
    elif option == 'Doctors':
        doctors()
